...
void mouseMotionDrag(int x, int y)
{
  ...
  if (g_iRightMouseButton) // handle camera rotations
  {
    ...
    if (Theta>pi_me / 2 - 0.01) // dont let the point enter the north pole
      Theta = pi_me / 2 - 0.01;
    
    if (Theta<- pi_me / 2 + 0.01)
      Theta = -pi_me / 2 + 0.01;
...
void keyboardFunc (unsigned char key, int x, int y)
{
  ...
  switch (key)
  {
    ...
    // camera control: reset the camera to the original position
    case 'e':
      Theta = pi_me / 6;
      Phi = pi_me / 6;
      viewingMode = 0;
